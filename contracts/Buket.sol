...
contract Buket is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 BUKET-WFTM LP -> BUKET pool
    uint256 public constant INITIAL_BUKET_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the BUKET ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BUKET", "BUKET") {
        // Mints 1 BUKET to contract creator for initial pool setup
    ...
    function _getBuketPrice() internal view returns (uint256 _buketPrice) {
        try IOracle(buketOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Buket: failed to fetch BUKET price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _buketPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_buketPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBuketOracle(address _buketOracle) public onlyOperatorOrTaxOffice {
        require(_buketOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BUKET to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BUKET to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBuketPrice = _getBuketPrice();
            currentTaxRate = _updateTaxRate(currentBuketPrice);
            if (currentBuketPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_buketPool != address(0), "!_buketPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_buketPool, INITIAL_BUKET_POOL_DISTRIBUTION);
